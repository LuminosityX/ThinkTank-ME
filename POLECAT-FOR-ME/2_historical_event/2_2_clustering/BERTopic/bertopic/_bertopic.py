...
class BERTopic:
    ...
    def __init__(self,
        ...
        self.set_output_path(save_path)

    def set_output_path(self, save_path):
        if not os.path.isdir(save_path):
            logger.info('Making new dir: ' + save_path)
            os.makedirs(save_path)
        self.save_path = save_path
        logger.info('Set save path: ' + save_path)
    ...
    def fit_time(self,
            ...
            images: List[str] = None,
            y: Union[List[int], np.ndarray] = None):
        self.fit_transform_time(documents=documents, embeddings=embeddings, time_embs=time_embs, y=y, images=images)
        return self

    def fit_time_umap(self,
            documents: List[str],
            embeddings: np.ndarray = None,
            time_embs: np.ndarray = None,
            ...
            images: List[str] = None,
            y: Union[List[int], np.ndarray] = None):
        self.fit_transform_time_umap(documents=documents, embeddings=embeddings, time_embs=time_embs,
                                     umap_embs=umap_embs, y=y, images=images)
        return self
    ...
    def fit_transform(self,
        ...
        ###
        # Normlization
        logger.info('start doc emb normalization')
        embeddings = normalize(embeddings, norm='l2')
        logger.info('Doc emb l2 normalized')

        # Reduce dimensionality
        if self.seed_topic_list is not None and self.embedding_model is not None:
            y, embeddings = self._guided_topic_modeling(embeddings)
        logger.info('start umap dimension reduction')
        umap_embeddings = self._reduce_dimensionality(embeddings, y)
        np.save(self.save_path + 'umap_embeddings.npy', umap_embeddings)
        logger.info('Umap embedding saved at: ' + self.save_path + 'umap_embeddings.npy')
        ...
        np.save(self.save_path + 'topic_probs.npy', self.probabilities_)
        logger.info('Topic probabilities saved at: ' + self.save_path + 'topic_probs.npy')
        logger.info('shape: ' + str(self.probabilities_.shape))

        json.dump(predictions, open(self.save_path + 'topic_predictions.json', 'w'), indent=4)
        logger.info('Topic prediction results saved at: ' + self.save_path + 'topic_predictions.json')
        logger.info('shape: ' + str(len(predictions)))

        return predictions, self.probabilities_

    def fit_transform_time(self,
                      documents: List[str],
                      embeddings: np.ndarray = None,
                      time_embs: np.ndarray = None,
                      images: List[str] = None,
    ...
    Union[np.ndarray, None]]:
        ...
        logger.info('Time feature added, saved at: '+ self.save_path + 'umap_embeddings_time.npy')
        logger.info('shape: ' + str(umap_embeddings_time.shape))

        # Cluster reduced embeddings
        logger.info('start hdbscan clustering')
        documents, probabilities = self._cluster_embeddings(umap_embeddings_time, documents, y=y)
    ...
    def fit_transform_time_umap(self,
                      documents: List[str],
                      embeddings: np.ndarray = None,
                      time_embs: np.ndarray = None,
                      umap_embs: np.ndarray = None,
    ...
    Union[np.ndarray, None]]:
        ...
        np.save(self.save_path + 'umap_embeddings.npy', umap_embeddings)
        logger.info('Umap embedding loaded and saved at: ' + self.save_path + 'umap_embeddings.npy')
        logger.info('shape: ' + str(umap_embeddings.shape))
